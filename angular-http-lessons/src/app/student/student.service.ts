...
export class StudentService {
  ...
  updatePartOfStudent(student: Student, id:number): Observable<Student> {
  ...
  searchStudents(term: string): Observable<Student[]> {
    ...
    return this.http.get<Student[]>(`${this.studentsUrl}/${term}`).pipe(
      tap(x => x.length ?
        this.log(`found heroes matching "${term}"`) :
        this.log(`no students matching "${term}"`)),
      catchError(this.handleError<Student[]>('searchLesson', []))
    );
  }
